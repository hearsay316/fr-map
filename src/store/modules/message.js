    ...
    mutations: {
        ...
        SET_TASK(state, { data, type }) {
            if (Array.isArray(data) && type === 'put') {
                ...
                // 回复指令
                const taskIndex = state.task.findIndex((item) => item.taskId === data.taskId); // 对应的任务
                if (
                    taskIndex !== -1 &&
                    ...
                    const teamIndex = state.task[taskIndex].chatroomTaskUserList.findIndex(
                    ...
                    state.task[taskIndex].chatroomTaskUserList[0].complete = data.complete;
                    state.task[taskIndex].chatroomTaskUserList[0].chatroomTaskUserReplyList.push({
                        ...data
                    });
                }
            } else if (type === 'add') {
        ...
        SET_RECONEWS(state, data) {
            if (checkType(data) === 'array') {
                ...
                if (data._type === 'add') {
                    ...
                    const ind = state.recoNews[data.posi].findIndex((t) => t.id === data.id);
                    if (ind !== -1) {
    ...
    actions: {
        ...
        connect_mqtt({ state, commit, dispatch }) {
            ...
            console.log(
                ...
                process.env.NODE_ENV === 'production'
                    ? mqttBase.clientId + Math.random().toString().slice(2, 7)
                    : process.env.VUE_APP_CLIENTID + Math.random().toString().slice(2, 7)
            );
        ...
        mqtt_handle_sub({ state }) {
            return new Promise((resolve, reject) => {
                ...
                if (status) {
                    state.client.unsubscribe(['ComplaintOrder', 'alarm/#'], { qos: 2 }, (err) => {
                        if (!err) {
                            console.log('取消订阅');
                            resolve();
                        } else {
            ...
            state.client.on('message', async function (topic, message) {
                ...
                switch (topic) {
                    ...
                    case 'alarm/continue':
                        ...
                        if (rootState.alarm.isStart && rootState.start.events.id === data.id) {
